...
def mk_binfiles(prefix_out,z0,zf,nz,sigma_thr=None,lmax=2000) :
    dz=(zf-z0)/nz
    z0arr=z0+dz*np.arange(nz)
    zfarr=z0arr+dz
    zmarr=z0arr+0.5*dz
    ...
    np.savetxt(prefix_out+".txt",np.transpose([z0arr,zfarr,szarr,larr]),
               fmt="%.3lE %.3lE %.3lE 0 0 %.3lE",
               header='[1]-z0 [2]-zf [3]-sz [4]-marg_sz [5]-marg_bz [6]-lmax')

nbins=[50,100,200]
for nb in nbins :
    mk_binfiles("bins_IM_z0p40_3p00_sthr0p75_n%d"%nb,0.4,3.00,nb,sigma_thr=0.75)
    mk_binfiles("bins_IM_z0p40_3p00_sthr0p50_n%d"%nb,0.4,3.00,nb,sigma_thr=0.50)
    mk_binfiles("bins_IM_z0p40_3p00_lmax2000_n%d"%nb,0.4,3.00,nb)
