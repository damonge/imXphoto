...
def get_bin_fnames(predir,expname,ibin,sthr) :
    ...
    if sthr==None :
        fname_bin_single=predir+"/bins_photoz_b%d_lmax5000_l5000.txt"%ibin
        fname_im=predir+"/bins_im_b%d_lmax5000_l5000.txt"%ibin
        fname_params=predir+"/params_b%d_"%ibin+expname+"_lmax5000_l5000"
        fname_fisher=fisher_prefix+expname+"_lmax5000_l5000"
    else :
        fname_bin_single=predir+"/bins_photoz_b%d_"%ibin+"sthr%.3lf_l5000.txt"%sthr
        fname_im=predir+"/bins_im_b%d_"%ibin+"sthr%.3lf_l5000.txt"%sthr
        fname_params=predir+"/params_b%d_"%ibin+expname+"_sthr%.3lf_l5000"%sthr
        fname_fisher=fisher_prefix+expname+"_sthr%.3lf_l5000"%sthr

    return fname_bin_single,fname_im,fname_params,fname_fisher

def prepare_bin(z0,zf,sz,lmx,predir,expname,ibin,lmax_im=5000,sthr=None,fac_sigma=3,fac_sample_sigma=2) :
...
def prepare_files(xp_photo,xp_im,bins_photo,sth_im,predir,fsky) :
    ...
    for i in np.arange(len(z0_ph_arr)) :
        ...
        write_param_file(xp_photo,xp_im,
                         ...
                         predir+"/output_l5000_b%d"%i,fname_params+".ini",fsky,fname_fisher)
        if run_cls :
...
def run_fsky(xp_photo,xp_im,bins_photo,sth_im,predir,fsky) :
    ...
    for i in np.arange(len(z0_ph_arr)) :
        ...
        write_param_file(xp_photo,xp_im,
                         fname_bins_photo,predir+"/bz_photoz_b%d.txt"%i,
                         fname_bins_im,predir+"/bz_im_b%d.txt"%i,
                         predir+"/output_l5000_b%d"%i,parname,fsky,fishname)
...
if run_cls :
    prepare_files(xp.phoz_LSSTgold,xp.im_HIRAX_32_6,"curves_LSST/bins_gold_lmax5000_l5000.txt",None,
                  "runs/IMAP",xp.im_HIRAX_32_6['fsky'])
else :
#    for exper in exper_array :
    for exper in [xp.im_HIRAX_32_6,xp.im_SKA,xp.im_MeerKAT] :
        prepare_files(xp.phoz_LSSTgold,exper,"curves_LSST/bins_gold_lmax5000_l5000.txt",None,
                      "runs/IMAP",exper['fsky'])
        prepare_files(xp.phoz_LSSTgold,exper,"curves_LSST/bins_gold_sthr0p50_l5000.txt",0.50,
                      "runs/IMAP",exper['fsky'])
        prepare_files(xp.phoz_LSSTgold,exper,"curves_LSST/bins_gold_sthr0p75_l5000.txt",0.75,
                      "runs/IMAP",exper['fsky'])
        prepare_files(xp.phoz_LSSTgold,exper,"curves_LSST/bins_gold_sthr1p00_l5000.txt",1.00,
                      "runs/IMAP",exper['fsky'])
